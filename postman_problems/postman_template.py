...
def get_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="distance",
        help='Edge attribute used to specify the distance between nodes (optional).Default is "distance".',
    )
    ...
    parser.add_argument(
        "--viz", action="store_true", help="Write out the static image of the CPP solution using graphviz?"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="dot",
        help='graphviz engine to use for viz layout: "dot", "neato", "fdp", etc) of solution animation viz',
    )
    ...
    parser.add_argument(
        "--fps", required=False, type=float, default=3, help="Frames per second to use for CPP solution animation."
    )
...
def generic_postman(postman_type):
    ...
    if args.animation:
        animation_images_dir = (
            args.animation_images_dir
            if args.animation_images_dir
            else os.path.join(os.path.dirname(args.animation_filename), "img")
        )

        logger.info("Creating individual files for animation...")
        message_images = make_circuit_images(
            circuit=circuit,
            graph=graph,
        ...
        message_animation = make_circuit_video(
            infile_dir_images=animation_images_dir,
